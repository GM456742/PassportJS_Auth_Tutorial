...
![](https://cdn.scotch.io/57241/0SiEsACSSa5teEke1eyb_projectTerminalNpmInit.PNG)
...
After this code runs, you should see two folders *models* and *config*. Open the config folder and you should see a *config.json* file. Open it and edit the development object's settings to match yours. If you have a password on your database enter it here in quotes. Example is below
```json
{
  "development": {
    "username": "root",
...
```<br/>
8. Let us go back to our *server.js* file and add a few lines of code. We will need to require the server to read the models folder and we will also need it to sync our inserts and reads to the database. Your server.js should look like this
```javascript
// Requiring necessary npm middleware packages 
var express = require("express");
var bodyParser = require("body-parser");
...
```<br/>
9. Now let us navigate to the config folder and create another folder called *middleware* and inside that folder create a file called *isAuthenticated.js*. Your should have `/config/middleware/isAuthenticated.js`
Open and edit the *isAuthenticated.js* file to match this
```javascript
...
10. It is time to set up passport. In the *config* folder create a file called passport.js. Open the file and input the following in the file. The comments explain it all.
```javascript
...
13. I created a sample public folder with its files in it. which is what i will be using for this tutorial. Download it from [Mediafire](http://www.mediafire.com/file/39hjqqp8t2ddyuy/learnPassportPublic.zip) and unzip into the root folder.
14. Take a look at the html files in the public folder. You will see that I captured the signup, login, and members page `GEt`s using `API`s. This way we can pass it to the server with ease.
15. Now open the *html-routes.js* and paste the following code
```javascript
